  ...
  return (
    <div className="rounded-2xl shadow p-4 flex gap-4 items-center">
      ...
      <div className="flex flex-col gap-2">
        <Link to={`/teachers/${id}`} className="px-4 py-2 rounded-xl bg-black text-white text-sm text-center">Xem chi tiết</Link>
        <Link to={`/teachers/${id}#book`} className="px-4 py-2 rounded-xl border text-sm text-center">Đặt học thử</Link>
      </div>
    </div>
  )
}
